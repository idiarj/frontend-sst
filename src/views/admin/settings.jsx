...
const Settings = () => {
  ...
  const filtrarReportes = () => {
    ...
    // Filtro por tipo de reporte (ejemplo simple)
    // Aquí podrías agregar lógica para semanal, mensual, anual según fechas
    return reportes;
  };
  ...
  const exportarPDF = () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text('Reporte de incidencias', 10, 10);
    let y = 20;
    reportesFiltrados.forEach((r, i) => {
      doc.setFontSize(12);
      doc.text(`#${i + 1} | Fecha: ${r.fecha} | Estado: ${r.estado} | Cédula: ${r.cedula}`, 10, y);
      y += 8;
      doc.text(`Descripción: ${r.descripcion}`, 10, y);
      y += 12;
      if (y > 270) {
        doc.addPage();
        y = 20;
          ...
          <div style={{ margin: '38px 0 0 0', maxWidth: 600 }}>
            ...
            >
              ...
              <button type="submit" style={{ marginLeft: 8, background: '#3b5998', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {/* Icono de lupa SVG de expovector.io */}
          ...
          <div style={{ marginTop: 32, maxWidth: 500 }}>
            ...
            <form
              ...
              onSubmit={e => {
                ...
                setApellido('');
                setCedula('');
                setErrorAgregar('Persona agregada correctamente (solo frontend)');
              }}
            >
              <input
                type="text"
